        ...
        VStack {
            Form {
                ...
                HStack(spacing: 30) {
                    Spacer(minLength: 0)
                    Button {
                        if let url = URL(string: "https://github.com/javieralas5/dinamic-island") {
            ...
            VStack(spacing: 0) {
                Divider()
                Text("Dynamic Island — macOS notch experience")
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
...
struct Advanced: View {
    ...
    @Default(.customAppIconData) var customAppIconData
    ...
    var body: some View {
        Form {
            ...
            Section {
                HStack(spacing: 16) {
                    Group {
                        if let data = customAppIconData, let img = NSImage(data: data) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Button("Upload icon") { pickCustomIcon() }
                            .buttonStyle(.borderedProminent)
                            .tint(Color.effectiveAccent)

                        if customAppIconData != nil {
                            Button("Reset to default") { resetIcon() }
                                .foregroundStyle(.secondary)
                                .font(.caption)
                                .buttonStyle(.plain)
                        }
    ...
    private func pickCustomIcon() {
        ...
        if #available(macOS 12.0, *) {
            panel.allowedContentTypes = [.png, .jpeg, .bmp, .tiff, .icns]
        }
        panel.prompt = "Use as Icon"
        guard panel.runModal() == .OK, let url = panel.url else { return }
        guard let image = NSImage(contentsOf: url) else { return }

        let resized = NSImage(size: NSSize(width: 512, height: 512))
        resized.lockFocus()
        ...
        Defaults[.customAppIconData] = pngData
        NSApp.applicationIconImage = NSImage(data: pngData)
    }

    private func resetIcon() {
        Defaults[.customAppIconData] = nil
        NSApp.applicationIconImage = NSImage(named: "logo2")
    }
